...
def load_train_dev_test(dataset):
    train = pd.read_csv(fr"../../BadActs-master/poison_data/{dataset}/1/synbkd/train-clean.csv",index_col=0)["0"].values.tolist()
    dev = pd.read_csv(fr"../../BadActs-master/poison_data/{dataset}/1/synbkd/dev-clean.csv",index_col=0)["0"].values.tolist()
    test = pd.read_csv(fr"../../BadActs-master/poison_data/{dataset}/1/synbkd/test-clean.csv",index_col=0)["0"].values.tolist()

    data = train + dev +test
    return data

def load_dataset_custom(path,label):
    loaded_text_label = pd.read_csv(path,index_col=0)
    data = loaded_text_label[loaded_text_label["1"] == label]["0"].values.tolist()
    return data
...
def load_train_set(dataset_name):
    # neg_yelp = load_dataset('../data/sentiment.train.0')
    # pos_yelp = load_dataset('../data/sentiment.train.1')

    neg_yelp = load_dataset_custom(fr'../../BadActs-master/poison_data/{dataset_name}/1/synbkd/train-clean.csv',0)
    pos_yelp = load_dataset_custom(fr'../../BadActs-master/poison_data/{dataset_name}/1/synbkd/train-clean.csv',1)
