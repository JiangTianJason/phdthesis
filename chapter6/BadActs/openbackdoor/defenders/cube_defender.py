...
from openbackdoor.victims import PLMVictim, Victim, GPTVictim, T5Victim
...
class CUBEDefender(Defender):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # self.encoder = PLMVictim(model=model_name, path=model_path, num_classes=num_classes)            ###The model is saved locally
        # self.encoder = GPTVictim(model="transformer", path=r"gpt2", num_classes=num_classes)                  ###The model is saved locally，Table 6.10
        self.encoder = T5Victim(model="transformer", path=r"flan-t5-base", num_classes=num_classes)        ###The model is saved locally，Table 6.10


        self.trainer = Trainer(warm_up_epochs=warm_up_epochs, epochs=epochs,
                                batch_size=batch_size, lr=lr,
                                save_path='./models/cube_t5_encoder_T5ClassificationHead', ckpt='last')
    ...
    def correct(
        ...
        # Step 1. Encoding
        embeddings, y_true = self.encode(poison_data,clean_data)
    ...
    def encode(self, dataset, clean_data):

        logger.info("Training encoder for CUBE defense")

        # self.encoder = self.trainer.train(self.encoder, {"train":dataset})
        self.encoder = self.trainer.train(self.encoder, {"train": dataset, "dev-clean": clean_data["dev-clean"],
                                                         "dev-poison": clean_data["dev-poison"]})           ###Use this line instead of the above original line to avoid Bugs
