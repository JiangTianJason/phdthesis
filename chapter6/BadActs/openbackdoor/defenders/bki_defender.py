...
import transformers
import torch
from openbackdoor.victims import Victim, PLMVictim, GPTVictim, T5Victim
...
class BKIDefender(Defender):
    ...
    def __init__(
        ...
        model_path: Optional[str] = r"/root/autodl-tmp/bert-base-uncased",
        **kwargs,
    ):
        ...
        # self.bki_model = PLMVictim(model=model_name, path=model_path, num_classes=num_classes)                                        ###The model is saved locally
        # self.bki_model = GPTVictim(model="transformer", path=r"/root/autodl-tmp/gpt2", num_classes=num_classes)                    ###The model is saved locally，Table 6.10
        self.bki_model = T5Victim(model="transformer", path=r"/root/autodl-tmp/flan-t5-base", num_classes=num_classes)       ###The model is saved locally，Table 6.10


        self.trainer = Trainer(warm_up_epochs=warm_up_epochs, epochs=epochs,
                                batch_size=batch_size, lr=lr,
                                save_path='./models/bki_t5_encoder', ckpt='last')
    ...
    def correct(
        ...
        self.bki_model = self.trainer.train(self.bki_model,{"train": poison_data, "dev-clean": clean_data["dev-clean"],"dev-poison": clean_data["dev-poison"]})     ###Use this line instead of the above original line to avoid Bugs
