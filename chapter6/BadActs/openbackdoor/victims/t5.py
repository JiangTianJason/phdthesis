...
class T5Victim(Victim):
    """
    T5 victims. Support Huggingface's Transformers.
    ...
    def get_repr_embeddings(self, inputs):
        decoder_input_ids = getattr(self.plm, self.model_name)._shift_right(inputs["input_ids"])
        output = getattr(self.plm, self.model_name)(**inputs,decoder_input_ids=decoder_input_ids).last_hidden_state  # batch_size, max_len, 768(1024)
        return output[:, 0, :]
