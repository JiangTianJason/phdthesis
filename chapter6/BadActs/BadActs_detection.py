# Defend “Inference-time” Poisoning attack。Section 6.3.2（1）：Table 6.1、6.2，Figure 6.4
import os
import time
import torch
import json
...
if __name__ == '__main__':
    for config_path in ['./configs/style_config.json','./configs/syntactic_config.json']:
        ...
        for model, path in [('bert',"bert-base-uncased")]:    ### the model is saved locally

            config["victim"]['model'] = model
            config["victim"]['path'] = path

            config.setdefault('defender', {})
            config['defender']['name'] = 'stylistic'                ###Our defense method "stylistic"
